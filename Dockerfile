...
RUN apt-get update
RUN apt-get install -y apache2 php php-mysql php-curl php-mdb2 php-mdb2-driver-mysql libapache2-mod-php openssl curl zip autoconf libmysqlclient-dev apache2-dev git inetutils-ping cron sudo wget vim
...
COPY . /var/www/html
RUN sudo rm -f index.html; chmod a+w phpOp/app.log; chmod a+w phpRp/app.log; mv start.sh /usr/local/bin/

...
include_once('includes/base64url.php');
...
function jwt_verify($jwt, $sig_hints = NULL) {
    ...
    for($i = 0; $i < $num_sigs; $i++) {
        ...
        switch($header['alg']) {
			...
			//TIM ajout pour le RSA avec sha 1
			
			case 'SHA1withRSA' :
			//log_debug("Signature Verification special TIM");
                $pems = array();
                $jwks = array();
                $vals = array();
                if($sig_hints) {
                ...
                foreach($pems as $pem) {
                    ...
                    $status = digest_verify_data("{$json_obj['header'][$i]}.{$json_obj['payload']}", $sig, $pubkeyid, 'sha1');
                    openssl_free_key($pubkeyid);
                    if($status) {
                        $verified = true;
                        break;
